...
{ 
 ...
 for($i=0;$i<$length;$i++) 
 { 
  $key .= $pattern{mt_rand(0,26)}; //生成php随机数 
 } 
 return $key; 
} 

function mac_seo_url($model,$param=[],$info=[],$str=[])
{
    foreach($param as $k=>$v){
        if(empty($v)){
            unset($param[$k]);
    ...
    switch ($model)
    {
        ...
        case 'vod/detail':
            ...
            if($config['view']['vod_detail'] == 2){
                ...
                $id = substr(md5($info['vod_en']),2,12);
...
function mac_seourl_vod_detail($info,$str)
{
    return mac_seo_url('vod/detail',[],$info,$str);
}
